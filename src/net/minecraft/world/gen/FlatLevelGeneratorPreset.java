...
public record FlatLevelGeneratorPreset(RegistryEntry<Item> displayItem, FlatChunkGeneratorConfig settings) {
    public static final Codec<FlatLevelGeneratorPreset> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)RegistryFixedCodec.of(RegistryKeys.ITEM).fieldOf("display").forGetter(preset -> preset.displayItem), (App)FlatChunkGeneratorConfig.CODEC.fieldOf("settings").forGetter(preset -> preset.settings)).apply((Applicative)instance, FlatLevelGeneratorPreset::new));
