...
extends AbstractNoiseBlockStateProvider {
    public static final Codec<NoiseThresholdBlockStateProvider> CODEC = RecordCodecBuilder.create(instance -> NoiseThresholdBlockStateProvider.fillCodecFields(instance).and(instance.group((App)Codec.floatRange((float)-1.0f, (float)1.0f).fieldOf("threshold").forGetter(noiseThresholdBlockStateProvider -> Float.valueOf(noiseThresholdBlockStateProvider.threshold)), (App)Codec.floatRange((float)0.0f, (float)1.0f).fieldOf("high_chance").forGetter(noiseThresholdBlockStateProvider -> Float.valueOf(noiseThresholdBlockStateProvider.highChance)), (App)BlockState.CODEC.fieldOf("default_state").forGetter(noiseThresholdBlockStateProvider -> noiseThresholdBlockStateProvider.defaultState), (App)Codec.list(BlockState.CODEC).fieldOf("low_states").forGetter(noiseThresholdBlockStateProvider -> noiseThresholdBlockStateProvider.lowStates), (App)Codec.list(BlockState.CODEC).fieldOf("high_states").forGetter(noiseThresholdBlockStateProvider -> noiseThresholdBlockStateProvider.highStates))).apply((Applicative)instance, NoiseThresholdBlockStateProvider::new));
