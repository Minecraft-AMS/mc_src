...
extends Feature<DefaultFeatureConfig> {
    ...
    private static void generateSuspiciousSand(StructureWorldAccess world, BlockPos pos2, Random random) {
        ...
        ObjectArrayList objectArrayList = Util.make(new ObjectArrayList(), positions -> {
            positions.add((Object)blockPos.east());
            positions.add((Object)blockPos.south());
            positions.add((Object)blockPos.west());
            positions.add((Object)blockPos.north());
        });
        ...
        Stream.concat(Stream.of(blockPos), objectArrayList.stream()).forEach(pos -> {
            if (mutableInt.getAndDecrement() > 0) {
                world.setBlockState((BlockPos)pos, Blocks.SUSPICIOUS_SAND.getDefaultState(), 3);
                ...
                world.setBlockState((BlockPos)pos, Blocks.SAND.getDefaultState(), 3);
            }
        });
    }
}
