...
public enum CellScaleLayer implements ParentedLayer
{
    ...
    public int sample(LayerSampleContext<?> context, LayerSampler parent, int x, int z) {
        ...
        if (v < t && v < u && v < w) {
            return parent.sample(this.transformX(m), this.transformZ(n + 4));
        }
        return parent.sample(this.transformX(m + 4), this.transformZ(n + 4)) & 0xFF;
    }
