...
public class WardenSpawnTrackerCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("warden_spawn_tracker").requires(source -> source.hasPermissionLevel(2))).then(CommandManager.literal("clear").executes(context -> WardenSpawnTrackerCommand.clearTracker((ServerCommandSource)context.getSource(), (Collection<? extends PlayerEntity>)ImmutableList.of((Object)((ServerCommandSource)context.getSource()).getPlayerOrThrow()))))).then(CommandManager.literal("set").then(CommandManager.argument("warning_level", IntegerArgumentType.integer((int)0, (int)4)).executes(context -> WardenSpawnTrackerCommand.setWarningLevel((ServerCommandSource)context.getSource(), (Collection<? extends PlayerEntity>)ImmutableList.of((Object)((ServerCommandSource)context.getSource()).getPlayerOrThrow()), IntegerArgumentType.getInteger((CommandContext)context, (String)"warning_level"))))));
    }
