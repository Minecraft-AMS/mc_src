...
extends ValueLookupTagProvider<GameEvent> {
    ...
    protected void configure(RegistryWrapper.WrapperLookup lookup) {
        ((ValueLookupTagProvider.ObjectBuilder)this.getOrCreateTagBuilder((TagKey)GameEventTags.VIBRATIONS)).add((T[])BASIC_GAME_EVENTS).add((GameEvent[])Vibrations.RESONATIONS).add(GameEvent.FLAP);
        ...
        ((ValueLookupTagProvider.ObjectBuilder)this.getOrCreateTagBuilder((TagKey)GameEventTags.WARDEN_CAN_LISTEN)).add((T[])BASIC_GAME_EVENTS).add((GameEvent[])Vibrations.RESONATIONS).add(GameEvent.SHRIEK).addTag((TagKey)GameEventTags.SHRIEKER_CAN_LISTEN);
