...
public class SculkSpreadManager {
    ...
    public void readNbt(NbtCompound nbt) {
        if (nbt.contains("cursors", 9)) {
            this.cursors.clear();
            List list = Cursor.CODEC.listOf().parse(new Dynamic((DynamicOps)NbtOps.INSTANCE, (Object)nbt.getList("cursors", 10))).resultOrPartial(arg_0 -> ((Logger)LOGGER).error(arg_0)).orElseGet(ArrayList::new);
    ...
    public void writeNbt(NbtCompound nbt) {
        Cursor.CODEC.listOf().encodeStart((DynamicOps)NbtOps.INSTANCE, this.cursors).resultOrPartial(arg_0 -> ((Logger)LOGGER).error(arg_0)).ifPresent(cursorsNbt -> nbt.put("cursors", (NbtElement)cursorsNbt));
    }
    ...
    public void tick(WorldAccess world, BlockPos pos2, Random random, boolean shouldConvertToBlock) {
        ...
        for (Cursor cursor : this.cursors) {
            ...
            object2IntMap.computeInt((Object)blockPos, (pos, charge) -> (charge == null ? 0 : charge) + cursor.charge);
    ...
    public static class Cursor {
        private static final ObjectArrayList<Vec3i> OFFSETS = Util.make(new ObjectArrayList(18), objectArrayList -> BlockPos.stream(new BlockPos(-1, -1, -1), new BlockPos(1, 1, 1)).filter(pos -> (pos.getX() == 0 || pos.getY() == 0 || pos.getZ() == 0) && !pos.equals(BlockPos.ORIGIN)).map(BlockPos::toImmutable).forEach(arg_0 -> ((ObjectArrayList)objectArrayList).add(arg_0)));
