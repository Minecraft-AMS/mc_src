...
public class TagPacketSerializer {
    public static Map<RegistryKey<? extends Registry<?>>, Serialized> serializeTags(DynamicRegistryManager dynamicRegistryManager) {
        return dynamicRegistryManager.streamSyncedRegistries().map(entry -> Pair.of(entry.key(), (Object)TagPacketSerializer.serializeTags(entry.value()))).filter(pair -> !((Serialized)pair.getSecond()).isEmpty()).collect(Collectors.toMap(Pair::getFirst, Pair::getSecond));
    }
    ...
    private static <T> Serialized serializeTags(Registry<T> registry) {
        ...
        registry.streamTagsAndEntries().forEach(pair -> {
            ...
            map.put(((TagKey)pair.getFirst()).id(), (IntList)intList);
        });
