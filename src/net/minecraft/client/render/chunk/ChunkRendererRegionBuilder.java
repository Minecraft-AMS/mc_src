...
public class ChunkRendererRegionBuilder {
    ...
    public ChunkRendererRegion build(World world, BlockPos startPos, BlockPos endPos, int offset) {
        ...
        for (int m = i; m <= k; ++m) {
            for (n = j; n <= l; ++n) {
                clientChunks[m - i][n - j] = (ClientChunk)this.chunks.computeIfAbsent(ChunkPos.toLong(m, n), pos -> new ClientChunk(world.getChunk(ChunkPos.getPackedX(pos), ChunkPos.getPackedZ(pos))));
            }
        }
