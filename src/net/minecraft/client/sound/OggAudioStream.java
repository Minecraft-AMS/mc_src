    ...
    private boolean readOggFile(ChannelList channelList) throws IOException {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            block14: {
                ...
                IntBuffer intBuffer2 = memoryStack.mallocInt(1);
                while (true) {
                    int i = STBVorbis.stb_vorbis_decode_frame_pushdata((long)this.pointer, (ByteBuffer)this.buffer, (IntBuffer)intBuffer, (PointerBuffer)pointerBuffer, (IntBuffer)intBuffer2);
                    this.buffer.position(this.buffer.position() + i);
                    int j = STBVorbis.stb_vorbis_get_error((long)this.pointer);
                    if (j == 1) {
                        this.increaseBufferSize();
