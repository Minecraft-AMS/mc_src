...
public class GlyphContainer<T> {
    ...
    @Nullable
    public T getGlyphs(int codePoint) {
        int i = codePoint >> 8;
    ...
    public T getOrInsert(int codePoint, IntFunction<T> ifAbsent) {
        int i = codePoint >> 8;
        T[] objects = this.rows[i];
        int j = codePoint & 0xFF;
        T object = objects[j];
    ...
    public IntSet getProvidedGlyphs() {
        ...
        this.forEachGlyph((codePoint, object) -> intOpenHashSet.add(codePoint));
