...
extends RealmsScreen {
    ...
    public void init() {
        ...
        this.player = -1;
        this.addDrawableChild(ButtonWidget.builder(Text.translatable("mco.configure.world.buttons.invite"), button -> this.client.setScreen(new RealmsInviteScreen(this.parent, this, this.serverData))).dimensions(i, RealmsPlayerScreen.row(1), this.column_width + 10, 20).build());
        this.removeButton = this.addDrawableChild(ButtonWidget.builder(Text.translatable("mco.configure.world.invites.remove.tooltip"), button -> this.uninvite(this.player)).dimensions(i, RealmsPlayerScreen.row(7), this.column_width + 10, 20).build());
        ...
        }).dimensions(i, RealmsPlayerScreen.row(9), this.column_width + 10, 20).build());
        this.addDrawableChild(ButtonWidget.builder(ScreenTexts.BACK, button -> this.backButtonClicked()).dimensions(i + this.column_width / 2 + 2, RealmsPlayerScreen.row(12), this.column_width / 2 + 10 - 2, 20).build());
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (this.serverData.players != null) {
            context.drawText(this.textRenderer, Text.empty().append(INVITED_TEXT).append(" (").append(Integer.toString(this.serverData.players.size())).append(")"), this.column1_x, RealmsPlayerScreen.row(0), 0xA0A0A0, false);
        } else {
            context.drawText(this.textRenderer, INVITED_TEXT, this.column1_x, RealmsPlayerScreen.row(0), 0xA0A0A0, false);
        }
    ...
    extends AlwaysSelectedEntryListWidget.Entry<InvitedObjectSelectionListEntry> {
        ...
        public InvitedObjectSelectionListEntry(PlayerInfo playerInfo) {
            ...
            this.uninviteButton = new TexturedButtonWidget(j, k, 8, 7, 0, 0, 7, CROSS_PLAYER_ICON, 8, 14, button -> RealmsPlayerScreen.this.uninvite(i));
            this.uninviteButton.setTooltip(Tooltip.of(REMOVE_TOOLTIP));
            this.buttons.add(this.uninviteButton);
            this.opButton = new TexturedButtonWidget(j += 11, k, 8, 7, 0, 0, 7, USER_ICON, 8, 14, button -> RealmsPlayerScreen.this.op(i));
            this.opButton.setTooltip(Tooltip.of(NORMAL_TOOLTIP));
            this.buttons.add(this.opButton);
            this.deopButton = new TexturedButtonWidget(j, k, 8, 7, 0, 0, 7, OP_ICON, 8, 14, button -> RealmsPlayerScreen.this.deop(i));
            this.deopButton.setTooltip(Tooltip.of(OPERATOR_TOOLTIP));
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            int i = !this.playerInfo.isAccepted() ? 0xA0A0A0 : (this.playerInfo.isOnline() ? 0x7FFF7F : 0xFFFFFF);
