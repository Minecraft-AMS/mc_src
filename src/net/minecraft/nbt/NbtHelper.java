...
public final class NbtHelper {
    ...
    private static final Splitter field_27819 = Splitter.on((char)':').limit(2);
    ...
    public static StringBuilder method_36116(StringBuilder stringBuilder, NbtElement nbtElement, int i, boolean bl) {
        switch (nbtElement.getType()) {
            ...
            case 10: {
                ...
                String string = Strings.repeat((String)" ", (int)j);
    ...
    static NbtCompound method_32275(NbtCompound compound) {
        ...
        Map map = (Map)nbtList.stream().map(NbtString.class::cast).map(NbtString::asString).collect(ImmutableMap.toImmutableMap(Function.identity(), NbtHelper::method_32267));
        ...
        if (i >= 0) {
            ...
            if (i + 2 <= string.length()) {
                ...
                COMMA_SPLITTER.split((CharSequence)string3).forEach(string2 -> {
                    List list = field_27819.splitToList((CharSequence)string2);
                    if (list.size() == 2) {
