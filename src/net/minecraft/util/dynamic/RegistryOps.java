...
extends ForwardingDynamicOps<T> {
    ...
    private RegistryOps(DynamicOps<T> delegate, EntryLoader entryLoader, DynamicRegistryManager registryManager, IdentityHashMap<RegistryKey<? extends Registry<?>>, ValueHolder<?>> valueHolders) {
        ...
        this.entryOps = delegate == JsonOps.INSTANCE ? this : new RegistryOps<T>(JsonOps.INSTANCE, entryLoader, registryManager, (IdentityHashMap<RegistryKey<Registry<?>>, ValueHolder<?>>)valueHolders);
    }
    ...
    private <E> DataResult<Supplier<E>> readSupplier(RegistryKey<? extends Registry<E>> key, final MutableRegistry<E> registry, Codec<E> codec, Identifier elementId) {
        ...
        valueHolder.values.put(registryKey, DataResult.success((Object)supplier));
        Optional optional = this.entryLoader.load((DynamicOps<JsonElement>)this.entryOps, key, registryKey, codec);
        if (!optional.isPresent()) {
            dataResult2 = DataResult.success((Object)new Supplier<E>(){
            ...
            DataResult dataResult3 = optional.get();
            Optional optional2 = dataResult3.result();
            if (optional2.isPresent()) {
                Pair pair2 = (Pair)optional2.get();
                registry.replace((OptionalInt)pair2.getSecond(), registryKey, pair2.getFirst(), dataResult3.lifecycle());
            }
    ...
    protected <E> DataResult<Registry<E>> getRegistry(RegistryKey<? extends Registry<E>> key) {
        return this.registryManager.getOptionalMutable(key).map(mutableRegistry -> DataResult.success((Object)mutableRegistry, (Lifecycle)mutableRegistry.getLifecycle())).orElseGet(() -> DataResult.error((String)("Unknown registry: " + key)));
    }
    ...
    public static interface EntryLoader {
                ...
                public <E> Optional<DataResult<Pair<E, OptionalInt>>> load(DynamicOps<JsonElement> dynamicOps, RegistryKey<? extends Registry<E>> registryId, RegistryKey<E> entryId, Decoder<E> decoder) {
                    ...
                    try (Resource resource = resourceManager.getResource(identifier2);){
                        ...
                        try (InputStreamReader reader = new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8);){
                            ...
                            optional = Optional.of(decoder.parse(dynamicOps, (Object)jsonElement).map(object -> Pair.of((Object)object, (Object)OptionalInt.empty())));
                        }
                        return optional;
                    }
                    catch (JsonIOException | JsonSyntaxException | IOException exception) {
                        return Optional.of(DataResult.error((String)("Failed to parse " + identifier2 + " file: " + exception.getMessage())));
                    }
                }
        ...
        implements EntryLoader {
            ...
            public <E> void add(DynamicRegistryManager.Impl registryManager, RegistryKey<E> key, Encoder<E> encoder, int rawId, E entry, Lifecycle lifecycle) {
                DataResult dataResult = encoder.encodeStart(RegistryReadingOps.of(JsonOps.INSTANCE, registryManager), entry);
            ...
            public <E> Optional<DataResult<Pair<E, OptionalInt>>> load(DynamicOps<JsonElement> dynamicOps, RegistryKey<? extends Registry<E>> registryId, RegistryKey<E> entryId, Decoder<E> decoder) {
                ...
                return Optional.of(decoder.parse(dynamicOps, (Object)jsonElement).setLifecycle(this.entryToLifecycle.get(entryId)).map(object -> Pair.of((Object)object, (Object)OptionalInt.of(this.entryToRawId.getInt((Object)entryId)))));
            }
        }
    }
