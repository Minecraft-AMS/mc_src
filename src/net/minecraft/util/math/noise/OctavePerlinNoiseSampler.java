...
implements NoiseSampler {
    ...
    public double sample(double x, double y, double z, double d, double e, boolean bl) {
        ...
        for (PerlinNoiseSampler perlinNoiseSampler : this.octaveSamplers) {
            f += perlinNoiseSampler.sample(OctavePerlinNoiseSampler.maintainPrecision(x * g), bl ? -perlinNoiseSampler.originY : OctavePerlinNoiseSampler.maintainPrecision(y * g), OctavePerlinNoiseSampler.maintainPrecision(z * g), d * g, e * g) / g;
            g /= 2.0;
        }
        return f;
    }
