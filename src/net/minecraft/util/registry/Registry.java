...
IndexedIterable<T> {
    ...
    public Codec<T> getCodec() {
        Codec codec = Identifier.CODEC.flatXmap(id -> Optional.ofNullable(this.get((Identifier)id)).map(DataResult::success).orElseGet(() -> DataResult.error((String)("Unknown registry key in " + this.registryKey + ": " + id))), value -> this.getKey(value).map(RegistryKey::getValue).map(DataResult::success).orElseGet(() -> DataResult.error((String)("Unknown registry element in " + this.registryKey + ":" + value))));
    ...
    public Codec<RegistryEntry<T>> createEntryCodec() {
        Codec codec = Identifier.CODEC.flatXmap(id -> this.getEntry(RegistryKey.of(this.registryKey, id)).map(DataResult::success).orElseGet(() -> DataResult.error((String)("Unknown registry key in " + this.registryKey + ": " + id))), entry -> entry.getKey().map(RegistryKey::getValue).map(DataResult::success).orElseGet(() -> DataResult.error((String)("Unknown registry element in " + this.registryKey + ":" + entry))));
