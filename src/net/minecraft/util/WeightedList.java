...
public class WeightedList<U> {
    ...
    public <T> T serialize(DynamicOps<T> ops, Function<U, Dynamic<T>> entrySerializer) {
        return (T)ops.createList(this.streamEntries().map(entry -> ops.createMap((Map)ImmutableMap.builder().put(ops.createString("data"), ((Dynamic)entrySerializer.apply(entry.getElement())).getValue()).put(ops.createString("weight"), ops.createInt(entry.getWeight())).build())));
    }
