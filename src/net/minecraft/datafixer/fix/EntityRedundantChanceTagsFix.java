...
extends DataFix {
    ...
    private static boolean hasZeroDropChance(OptionalDynamic<?> optionalDynamic, int i) {
        return optionalDynamic.flatMap(arg_0 -> field_25695.parse(arg_0)).map(list -> list.size() == i && list.stream().allMatch(float_ -> float_.floatValue() == 0.0f)).result().orElse(false);
    }
}
