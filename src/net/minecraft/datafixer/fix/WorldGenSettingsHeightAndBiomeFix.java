...
extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped(NAME, type, type2, typed2 -> {
            OptionalDynamic optionalDynamic = ((Dynamic)typed2.get(DSL.remainderFinder())).get(HAS_INCREASED_HEIGHT_ALREADY_KEY);
            ...
            return typed2.update(DSL.remainderFinder(), dynamic -> dynamic.remove(HAS_INCREASED_HEIGHT_ALREADY_KEY)).updateTyped(opticFinder, type3, typed -> {
                Dynamic dynamic2 = (Dynamic)typed.write().result().orElseThrow(() -> new IllegalStateException("Malformed WorldGenSettings.dimensions"));
                ...
                return (Typed)((Pair)type3.readTyped(dynamic2).result().orElseThrow(() -> new IllegalStateException("WorldGenSettingsHeightAndBiomeFix failed."))).getFirst();
            });
        });
    }
