        ...
        return TypeRewriteRule.seq((TypeRewriteRule)this.fixTypeEverywhereTyped("ChunkToProtoChunkFix", type, this.getOutputSchema().getType(TypeReferences.CHUNK), typed -> typed.updateTyped(opticFinder, type4, typed2 -> {
            ...
            if (bl) {
                ...
                if (optional.isPresent()) {
                    ...
                    dynamic2 = dynamic2.set("ToBeTicked", dynamic2.createList(list.stream().map(shortList -> dynamic22.createList(shortList.stream().map(arg_0 -> ((Dynamic)dynamic22).createShort(arg_0))))));
                }
                dynamic3 = (Dynamic)DataFixUtils.orElse((Optional)typed2.set(DSL.remainderFinder(), (Object)dynamic2).write().result(), (Object)dynamic2);
            } else {
                dynamic3 = dynamic2;
            }
