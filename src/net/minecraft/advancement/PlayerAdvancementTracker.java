...
public class PlayerAdvancementTracker {
    ...
    private void load(ServerAdvancementLoader advancementLoader) {
        if (Files.isRegularFile(this.filePath, new LinkOption[0])) {
            try (JsonReader jsonReader = new JsonReader((Reader)Files.newBufferedReader(this.filePath, StandardCharsets.UTF_8));){
    ...
    public void save() {
        ...
        jsonElement.getAsJsonObject().addProperty("DataVersion", (Number)SharedConstants.getGameVersion().getSaveVersion().getId());
        try {
