...
public class WardenBrain {
    ...
    private static void addCoreActivities(Brain<WardenEntity> brain) {
        brain.setTaskList(Activity.CORE, 0, (ImmutableList<Task<WardenEntity>>)ImmutableList.of((Object)new StayAboveWaterTask(0.8f), LookAtDisturbanceTask.create(), (Object)new LookAroundTask(45, 90), (Object)new WanderAroundTask()));
    }
    ...
    private static void addInvestigateActivities(Brain<WardenEntity> brain) {
        brain.setTaskList(Activity.INVESTIGATE, 5, (ImmutableList<Task<WardenEntity>>)ImmutableList.of(FindRoarTargetTask.create(WardenEntity::getPrimeSuspect), WalkTowardsPosTask.create(MemoryModuleType.DISTURBANCE_LOCATION, 2, 0.7f)), MemoryModuleType.DISTURBANCE_LOCATION);
    }
    ...
    private static void addFightActivities(WardenEntity warden, Brain<WardenEntity> brain) {
        brain.setTaskList(Activity.FIGHT, 10, (ImmutableList<Task<WardenEntity>>)ImmutableList.of(RESET_DIG_COOLDOWN_TASK, ForgetAttackTargetTask.create(entity -> !warden.getAngriness().isAngry() || !warden.isValidTarget((Entity)entity), WardenBrain::removeDeadSuspect, false), LookAtMobTask.create((LivingEntity entity) -> WardenBrain.isTargeting(warden, entity), (float)warden.getAttributeValue(EntityAttributes.GENERIC_FOLLOW_RANGE)), RangedApproachTask.create(1.2f), (Object)new SonicBoomTask(), MeleeAttackTask.create(18)), MemoryModuleType.ATTACK_TARGET);
    }
