...
public class FindPointOfInterestTask {
    ...
    public static Task<PathAwareEntity> create(Predicate<RegistryEntry<PointOfInterestType>> poiPredicate, MemoryModuleType<GlobalPos> poiPosModule, MemoryModuleType<GlobalPos> potentialPoiPosModule, boolean onlyRunIfChild, Optional<Byte> entityStatus) {
        ...
        SingleTickTask<PathAwareEntity> singleTickTask = TaskTriggerer.task(arg_0 -> FindPointOfInterestTask.method_46879(potentialPoiPosModule, onlyRunIfChild, mutableLong, (Long2ObjectMap)long2ObjectMap, poiPredicate, entityStatus, arg_0));
    ...
    private static /* synthetic */ App method_46879(MemoryModuleType context, boolean bl, MutableLong mutableLong, Long2ObjectMap long2ObjectMap, Predicate predicate, Optional optional, TaskTriggerer.TaskContext taskContext) {
        return taskContext.group(taskContext.queryMemoryAbsent(context)).apply((Applicative)taskContext, queryResult -> (world, entity, time) -> {
            ...
            Set<Pair<RegistryEntry<PointOfInterestType>, BlockPos>> set = pointOfInterestStorage.getSortedTypesAndPositions(predicate, predicate2, entity.getBlockPos(), 48, PointOfInterestStorage.OccupationStatus.HAS_SPACE).limit(5L).collect(Collectors.toSet());
            ...
            if (path != null && path.reachesTarget()) {
                ...
                for (Pair<RegistryEntry<PointOfInterestType>, BlockPos> pair : set) {
                    long2ObjectMap.computeIfAbsent(((BlockPos)pair.getSecond()).asLong(), m -> new RetryMarker(serverWorld.random, time));
                }
            }
            return true;
        });
