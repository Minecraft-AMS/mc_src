...
public class GoToRememberedPositionTask {
    ...
    private static <T> SingleTickTask<PathAwareEntity> create(MemoryModuleType<T> posSource, float speed, int range, boolean requiresWalkTarget, Function<T, Vec3d> posGetter) {
        return TaskTriggerer.task(context -> context.group(context.queryMemoryOptional(MemoryModuleType.WALK_TARGET), context.queryMemoryValue(posSource)).apply((Applicative)context, (walkTarget, posSource) -> (world, entity, time) -> {
            Vec3d vec3d4;
            Vec3d vec3d3;
            Vec3d vec3d2;
