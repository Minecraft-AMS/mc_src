...
public class Memory<T> {
    ...
    public static <T> Codec<Memory<T>> createCodec(Codec<T> codec) {
        return RecordCodecBuilder.create(instance -> instance.group((App)codec.fieldOf("value").forGetter(memory -> memory.value), (App)Codec.LONG.optionalFieldOf("ttl").forGetter(memory -> memory.isTimed() ? Optional.of(memory.expiry) : Optional.empty())).apply((Applicative)instance, (value, expiry) -> new Memory<Object>(value, expiry.orElse(Long.MAX_VALUE))));
    }
}
