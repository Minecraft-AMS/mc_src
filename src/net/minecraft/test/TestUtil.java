...
public class TestUtil {
    ...
    public static Collection<GameTestBatch> createBatches(Collection<TestFunction> testFunctions) {
        ...
        return (Collection)map.entrySet().stream().flatMap(entry -> {
            ...
            return Streams.stream((Iterable)Iterables.partition((Iterable)collection, (int)100)).map(testFunctions -> new GameTestBatch(string + ":" + mutableInt.incrementAndGet(), (Collection<TestFunction>)ImmutableList.copyOf((Collection)testFunctions), consumer, consumer2));
